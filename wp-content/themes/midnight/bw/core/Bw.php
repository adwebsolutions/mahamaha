...
class Bw {
    ...
    static function ot_url( $string ) {
        if ( strpos( $string, 'OT_THEME_DIR') !== false ) {
            $string = preg_replace( "#^[^:/.]*[:/]+#i", "", $string );
            $string = str_replace( "OT_THEME_DIR", get_template_directory_uri(), $string );
    ...
    static function the_breadcrumb() {
        ...
        if ( ! is_front_page() ) {
            ...
            _e('Home','midnight');
            echo "</a></li>";
            if( function_exists('is_product_category') and is_product_category() ) {
