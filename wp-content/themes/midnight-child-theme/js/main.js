...
var App = {
    ...
    newsletter: function() {
        ...
        if( $nlsModal.length && ! $nlsModal.hasClass('bw-dont-trigger') ) {
            setTimeout(function() {
                if( $nlsModal.hasClass('bw-show-once') ) {
                    if( localStorage.getItem( 'bwNewsletter4') == null ) {
                        App.modal.triggerModal( 'bw-nsl' );
                        localStorage.setItem( 'bwNewsletter4', new Date().getTime() );
                    }else{
                        var hours = 24*30;
                        var now = new Date().getTime();
                        if( now - localStorage.getItem('bwNewsletter4') > hours*60*60*1000 ) {
                            localStorage.clear();
                            App.modal.triggerModal( 'bw-nsl' );
                            localStorage.setItem( 'bwNewsletter4', now );
                        }
                    }
                }else{
                    App.modal.triggerModal( 'bw-nsl' );
    ...
    woocommerce: {
        ...
        bind: function() {
            ...
            $('.woocommerce-cart .bw-quantity').on('click', function() {
            ...
            // bw-featured-cat-products
            $('.bw-featured-cat-tabs li').on('click', this.featuredCatProducts);
        ...
        featuredCatProducts: function() {

            "use strict";

            var $this = $(this), $featured = $this.closest('.bw-featured-cat-products'), $featuredTabs = $this.closest('.bw-featured-cat-tabs').find('li'), $featuredList = $featured.find('ul.products');
            ...
            var productData = {
                'action'            : '__call_featured_cat_products',
                'tab'               : $this.attr('data-tab'),
                'number_of_posts'   : $featured.attr('data-number_of_posts'),
                'items_per_row'     : $featured.attr('data-items_per_row')
            };

            if( typeof $featured.attr('data-param') !== 'undefined' ) { productData.params = $featured.attr('data-param'); }
